...
object DatabaseModule {
    ...
    /**
     * Migration from version 28 to 29 (Serie A — mapping canali per squadra):
     * - Creates `serie_a_team_channels` (persistent team→channels links).
     */
    private val MIGRATION_28_29 = object : Migration(28, 29) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS serie_a_team_channels (
                    teamTla TEXT NOT NULL,
                    channelStreamUrl TEXT NOT NULL,
                    teamShortName TEXT NOT NULL,
                    channelName TEXT NOT NULL,
                    savedAt INTEGER NOT NULL,
                    PRIMARY KEY(teamTla, channelStreamUrl)
                )
            """)
        }
    }
